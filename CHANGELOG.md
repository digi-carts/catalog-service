...
All notable changes to **catalog-service** are documented in this file.
...
- `@Autowired` on `ProductService`'s two-argument constructor so Spring Boot tests pick it over the package-private `RestClient` constructor
...
### Changed
- Product create still succeeds if platform-service is unreachable; the failure is logged at debug (Sonar S1166)
- Product GET-by-id `Optional` mapping is explicitly typed so the module compiles on JDK 24
